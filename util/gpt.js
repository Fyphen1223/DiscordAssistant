...
class webSummarizer {
	...
	summarize = async function (content) {
		const res = await generate(JSON.stringify({
			'messages': [{
				'role': 'user',
				'content': 'Your mission is to delete ad text from the web-page\'s text that user gives you.You should not delete some type of content such as code, sicne this can affect proper actions. Return ONLY shortend content.',
			}],
			'prompt': content,
			'model': 'GPT-4',
			'markdown': false,
